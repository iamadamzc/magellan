...
**The Magellan project maintains a strict 4-stage strategy lifecycle pipeline. Code moves linearly from development to production.**

`Dev` -> `Test` -> `Deployable` -> `Deployed`

---

## Root Directory Rules

### ✅ ALLOWED in Root (12 Essential Items Only)
...
7.  `DIRECTORY_STRUCTURE_RULES.md` - This file
8.  `main.py` - Application entry point
9.  `requirements.txt` - Python dependencies
10. `debug_vault.log` / `livetradelog.txt` - Runtime logs
11. **Pipeline Directories**: `dev/`, `test/`, `deployable_strategies/`, `deployed/`
12. **System Directories**: `system_docs/`, `research/`, `src/`, `scripts/`

### ❌ NEVER ALLOWED in Root

*   **Session summaries** → Move to `system_docs/session_logs/`
*   **Strategy guides** → Move to `deployable_strategies/{strategy}/README.md`
*   **Test scripts** → Move to `research/testing/` or `dev/`
*   **CSV result files** → Move to `research/testing/backtests/`
*   **Any .md file except allowed root docs**

---

## Strategy Lifecycle Pipeline

### 1. `/dev/` - Development & Prototyping
**Purpose:** Experimental sandbox for new strategies or major refactors.
**Rules:**
*   ✅ Rapid iteration allowed
*   ✅ No strict validation required yet
*   ❌ Do NOT use for production trading

### 2. `/test/` - Validation & Tuning
**Purpose:** Candidates for production verification. Strategies here are undergoing backtesting, WFA, or paper trading calibration.
**Rules:**
*   ✅ Must have `README.md` validation plan
*   ✅ working implementation of `strategy.py`
*   ❌ No hardcoded API keys

### 3. `/deployable_strategies/` - Production Ready (Code)
**Purpose:** The "Gold Master" codebase. Strategies here are fully validated and signed off for deployment.
**Rules:**
*   ✅ **IMMUTABLE CODE**: Changes require a full PR/validation cycle
*   ✅ Contains `runner.py`, `strategy.py`, and default `config.json`
*   ❌ NO deployment-specific state (e.g. specific account keys, live logs)
*   **Key Requirement**: `runner.py` must accept `CONFIG_PATH` env var to separate code from config.

### 4. `/deployed/` - Live Production State (Config)
**Purpose:** The actual configuration and service definitions for strategies running on EC2.
**Structure:**
```
/deployed/
├── {strategy_name}/
│   ├── config.json              # The ACTUAL config used in prod (specific accounts)
│   ├── magellan-{strat}.service # Systemd service definition
│   └── logs/ (optional)
```
**Rules:**
*   ✅ Maps 1:1 with active systemd services
*   ✅ Points to code in `/deployable_strategies/` via `ExecStart`
*   ❌ No Python code (configs only)

---

## Support Directories

### `/system_docs/` - Centralized Documentation
*   `architecture/`: System design
*   `operations/`: Runbooks, CI/CD guides
*   `reference/`: Knowledge base
*   `session_logs/`: Archive of session summaries

### `/research/` - Deep Dives
*   `testing/`: Backtests, perturbations, ML
*   `analysis/`: Jupyter notebooks, market studies

### `/src/` - Core Libraries
*   Shared code used by multiple strategies (e.g. data handlers, utils).
*   **Note**: Strategies should import from here, not duplicate code.

### `/scripts/` - Utilities
*   Maintenance scripts, dashboards (e.g. `monitor_dashboard.py`), data fetchers.
...
1.  **Scan Root**: Ensure no loose files.
2.  **Verify Pipeline**: Ensure strategies are in the correct lifecycle folder (`dev`, `test`, or `deployable`).
3.  **Check Documentation**: Ensure `system_docs` is organized.
4.  **No Duplicates**: Remove older versions of files if a newer one exists in a promoted folder.
...
Violations must be fixed immediately. This structure guarantees that **Code** (deployable) is verified separately from **Configuration** (deployed), preventing production accidents.
