"""
Phase 2 Validation: System 4 on NVDA - Fixed-Duration Options Momentum

Testing NVDA (high-beta tech stock) instead of SPY:
- NVDA has 2.5x volatility vs SPY
- Longer trend persistence (30-60 day moves)
- 2025 data only (avoids June 2024 stock split)

Strategy (SAME as SPY System 4):
- Entry: RSI 65/35 (high conviction momentum)
- Hold: EXACTLY 45-60 days (target 52 DTE at entry)
- Exit: ONLY when DTE < 7 (roll to new contract)
- NO stop loss, NO signal exits!

Hypothesis: NVDA's higher volatility will generate enough intrinsic
value growth to overcome theta decay (which SPY couldn't do).

Run: python research/backtests/options/phase2_validation/test_nvda_system4.py
...
class NVDAOptionsBacktester:
    """
    Backtest NVDA options with System 4 (Fixed-Duration Momentum).
    
    Testing high-beta stock (NVDA) vs low-beta index (SPY).
    
    Strategy:
    - RSI > 65: Buy CALL (delta 0.70, 52 DTE)
    - RSI < 35: Buy PUT (delta 0.70, 52 DTE)
    - Hold EXACTLY until DTE < 7, then ROLL
    - NO other exits!
    """
    ...
    def fetch_nvda_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        """Fetch NVDA daily OHLCV data."""
        LOG.flow("[DATA] Fetching NVDA daily bars...")
        ...
        if df is None or len(df) == 0:
            raise ValueError("Failed to fetch NVDA data")
...
def main():
    """Run System 4 on NVDA (high-beta tech stock)."""
    print("=" * 70)
    print("SYSTEM 4: NVDA OPTIONS MOMENTUM (2025 Test)")
    print("=" * 70)
    print()
    print("Asset: NVDA (avoiding June 2024 split, testing 2025 only)")
    ...
    # Date range (2025 only - avoids June 2024 NVDA stock split)
    start_date = '2025-01-01'
    end_date = '2026-01-15'
    
    # Run backtest
    backtester = NVDAOptionsBacktester(config)
    
    # Fetch data
    df = backtester.fetch_nvda_data(start_date, end_date)
    ...
    print(f"\n📈 vs NVDA Buy & Hold:")
    print(f"  NVDA Buy & Hold: {metrics['spy_buy_hold_pct']:+.2f}%")
    ...
    results['equity_curve'].to_csv(output_dir / 'nvda_system4_equity_curve.csv')
    results['trades'].to_csv(output_dir / 'nvda_system4_trades.csv', index=False)
