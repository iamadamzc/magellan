"""
Phase 2 Validation: System 3 - Options Momentum Breakout

Tests high-conviction options strategy on SPY (2024-2026):
- Entry: RSI 65/35 (vs 58/42 for System 1) - TIGHTER SIGNALS
- Exit: RSI 48-52 for 2 days (mean reversion with confirmation)
- Stop Loss: RSI crosses back through entry threshold
- Delta: 0.70 (proven optimal from parameter sweep)
- DTE: 45-60 days
- Slippage: 1.0%

Improvements over System 1 Baseline:
- Higher conviction signals → Better win rate (target >50% vs 28%)
- Fewer trades → Less theta decay (target 12-18/year vs 28/year)
- Active exits → Capture gains before mean reversion

Success Criteria:
- Sharpe ratio > 1.0 (vs 0.55 baseline)
- Win rate > 50% (vs 28% baseline)
- Total return > 0%
- Trades: 10-20 for 2 years

Run: python research/backtests/options/phase2_validation/test_system3_momentum.py
...
class SPYOptionsBacktester:
    """
    Backtest SPY options strategy with System 3 (Options Momentum Breakout) signals.
    
    Strategy:
    - RSI > 65: Buy CALL (delta 0.70, 45-60 DTE) - HIGH CONVICTION
    - RSI < 35: Buy PUT (delta 0.70, 45-60 DTE) - HIGH CONVICTION
    - 48 <= RSI <= 52 for 2 days: Close position (mean reversion)
    - Stop loss: RSI crosses back through entry threshold
    ...
    def __init__(self, config: dict):
        ...
        LOG.event(f"[SYSTEM 3] Initialized Options Momentum Breaktest")
    ...
    def calculate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate RSI and generate BUY/SELL/HOLD signals for System 3.
        
        System 3 Logic (Options Momentum Breakout):
        - Entry: RSI > 65 (calls) or RSI < 35 (puts) - HIGH CONVICTION
        - Exit: RSI 48-52 for 2 consecutive days (mean reversion confirmation)
        - Stop Loss: RSI crosses back through entry threshold (signal invalidation)
        
        Returns:
            DataFrame with 'rsi' and 'signal' columns
        """
        LOG.flow("[SYSTEM 3] Calculating RSI and momentum breakout signals...")
        ...
        for idx, row in df.iterrows():
            ...
            elif current_position == 'BUY' and rsi < self.rsi_sell_threshold:  # Default: 35
                # CALL signal invalidated (RSI back below entry threshold)
                current_position = 'HOLD'
                rsi_neutral_days = 0
            
            elif current_position == 'SELL' and rsi > self.rsi_buy_threshold:  # Default: 65
    ...
    def simulate_options_trading(self, df: pd.DataFrame) -> dict:
        ...
        """
        LOG.event("[SYSTEM 3] Starting simulation...\n")
...
def main():
    """Run System 3 (Options Momentum Breakout) backtest."""
    print("=" * 70)
    print("SYSTEM 3: OPTIONS MOMENTUM BREAKOUT (Phase 2 Validation)")
    print("=" * 70)
    print()
    ...
    results['equity_curve'].to_csv(output_dir / 'spy_system3_equity_curve.csv')
    results['trades'].to_csv(output_dir / 'spy_system3_trades.csv', index=False)
