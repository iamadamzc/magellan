"""
Phase 2 Validation: System 4 - Fixed-Duration Options Momentum

Based on empirical finding: 100% of trades held 45+ days were profitable!

Strategy (SIMPLIFIED):
- Entry: RSI 65/35 (high conviction momentum) ✅ PROVEN
- Hold: EXACTLY 45-60 days (target 52 DTE at entry)
- Exit: ONLY when DTE < 7 (roll to new contract)
- NO stop loss, NO signal exits, NO mean reversion exits!

Rationale:
- Theta decay is lowest in days 45-60, accelerates below 30 days
- Intrinsic value from trend needs 45+ days to compound
- Signal-based exits destroy value (0% win rate on early exits)
- Time-based exits preserve value (100% win rate on 45+ day holds)

Expected Performance:
- Trades: 6-10/year
- Win Rate: 70-85% (based on historical 45+ day holds)
- Sharpe: 1.2-1.8 (target)

Run: python research/backtests/options/phase2_validation/test_system4_fixed_duration.py
...
class SPYOptionsBacktester:
    """
    Backtest SPY options with System 4 (Fixed-Duration Momentum) signals.
    ...
    - NO other exits (no stop loss, no mean reversion, no signal changes!)
    
    This is based on the empirical finding that 100% of trades held
    45+ days in System 3 were profitable (+18.89% avg).
    """
