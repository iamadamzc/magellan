...
for year in range(2020, 2026):
    ...
    if len(year_trades) > 0:
        ...
        sharpe = (trade_returns.mean() / trade_returns.std() * np.sqrt(len(year_trades))) if trade_returns.std() > 0 else 0
...
overall_sharpe = (all_trades_df['pnl_pct'].mean() / all_trades_df['pnl_pct'].std() * np.sqrt(len(all_trades))) if all_trades_df['pnl_pct'].std() > 0 else 0
