"""
V3 Option B Backtest - Balanced Configuration

Parameters:
- VWAP Threshold: 0.50% (sweet spot between V1 and V2)
- Profit Target: 0.30%
- Stop Loss: 0.20% (NEW)
- Time Filter: 12-2 PM (keep)
- Volume Filter: >1.5x (RELAXED from 2x)
- NO ATR filter (allow all volatility regimes)

Expected: ~130 trades/year, 48-52% win rate, Sharpe 0.5-2.0
"""
...
def v3_option_b_strategy(df, vwap_thresh=0.50, profit_target=0.30, stop_loss=0.20, 
                         hold_minutes=15, vol_multiplier=1.5):
    """
    V3 Option B: Balanced approach
    - Moderate threshold (0.50%)
    - Relaxed volume filter (1.5x)
    - No ATR filter
    - Time filter only
    """
    if len(df) < 30:
...
print("="*80)
print("V3 OPTION B BACKTEST - 2024 & 2025 (FULL YEARS)")
print("="*80)
print("\nConfiguration:")
print("  VWAP Threshold: 0.50% (balanced)")
print("  Profit Target: 0.30%")
print("  Stop Loss: 0.20%")
print("  Hold Time: 15 min")
print("  Time Filter: Avoid 12-2 PM")
print("  Volume Filter: >1.5x average (RELAXED)")
print("  Volatility Filter: NONE")
...
for year, days in [('2024', days_2024), ('2025', days_2025)]:
    ...
    for idx, date in enumerate(days):
        ...
        trades = v3_option_b_strategy(df)
    ...
    if all_trades:
        ...
        print(f"\n⚠️  No trades in {year}")
        results_v3[year] = None

# Final comparison
print(f"\n{'='*80}")
print("VERSION COMPARISON (2024)")
print(f"{'='*80}")
...
print(f"\n{'Metric':<20s} | {'V1 (0.45%)':>15s} | {'V3B (0.50%)':>15s} | {'Change':>12s}")
print("-" * 70)
...
for metric, label in [
    ...
    v3_val = results_v3['2024'].get(metric, 0) if results_v3.get('2024') else 0
    change = v3_val - v1_val
    
    if metric == 'total_trades':
        print(f"{label:<20s} | {v1_val:>15.0f} | {v3_val:>15.0f} | {change:>+12.0f}")
    else:
        print(f"{label:<20s} | {v1_val:>15.2f} | {v3_val:>15.2f} | {change:>+12.2f}")

# Save
with open('v3_option_b_results.json', 'w') as f:
    json.dump(results_v3, f, indent=2, default=str)

print(f"\n{'='*80}")
print("✅ Saved V3 Option B results to v3_option_b_results.json")
print(f"{'='*80}")
