"""
MA Crossover Strategy (Daily, MAG7 Equities)

Classic moving average crossover - baseline for comparison:
- Entry: 20 MA crosses above 50 MA
- Exit: 20 MA crosses below 50 MA

Simple trend following without filters
"""
...
def run_backtest(symbol, period_name, start, end, friction_bps=1.5):
    ...
    # Entry: 20 MA crosses above 50 MA
    entry_condition = (df['ma_20'] > df['ma_50']) & (df['ma_20'].shift(1) <= df['ma_50'].shift(1))
    
    # Exit: 20 MA crosses below 50 MA
    exit_condition = (df['ma_20'] < df['ma_50']) & (df['ma_20'].shift(1) >= df['ma_50'].shift(1))
...
def main():
    parser = argparse.ArgumentParser(description='Test MA Crossover Strategy')
