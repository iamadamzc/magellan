"""
ORB V10E - Wider Targets
-------------------------
V10D: 72% win rate but -0.162% avg (stops -0.955%, scales +0.176%/+0.438%)

NEW APPROACH: Keep stop at OR low, but scale at 0.5R/1.0R instead of 0.25R/0.5R
This doubles the profit targets while keeping same risk
"""
...
def run_orb_v10e(symbol, start, end):
    """ORB V10E - Wider targets (0.5R/1.0R/2.0R)"""
    ...
    params = {
        ...
        'SCALE_100R_PCT': 0.25,      # Scale 25% at 1.0R
        'BREAKEVEN_TRIGGER_R': 0.5,  # Move to BE at 0.5R
        'TARGET_20R': 2.0,           # Target 2.0R for runner
        'TRAIL_ATR': 0.4,
    }
    ...
    for i in range(len(df)):
        ...
        if position is None:
            ...
            # Scale 25% @ 1.0R
            if current_r >= 1.0 and not scaled_100r:
                pnl_pct = (risk * 1.0) / entry_price * 100 * params['SCALE_100R_PCT']
                trades.append({
                    'symbol': symbol,
                    'entry_time': entry_time,
                    'exit_time': current_time,
                ...
                position -= params['SCALE_100R_PCT']
                scaled_100r = True
            
            # Target @ 2.0R
            if current_r >= params['TARGET_20R'] and position > 0:
                pnl_pct = (risk * params['TARGET_20R']) / entry_price * 100 * position
                trades.append({
                    'symbol': symbol,
                    'entry_time': entry_time,
                    'exit_time': current_time,
