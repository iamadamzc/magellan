"""
ORB V13b "MINIMAL" - Isolated VWAP Exit Removal
------------------------------------------------
SINGLE CHANGE FROM V7: Remove VWAP loss exit only

This is the MINIMAL test to isolate the core hypothesis:
"VWAP loss exit is killing winners"

ALL OTHER PARAMETERS IDENTICAL TO V7:
- Breakeven @ 0.5R (keep V7)
- Scale 50% @ 1.3R (keep V7)
- Trail 0.6 ATR (keep V7)
- NO time stop (keep V7 behavior)
- NO volatility filter (keep V7)
- Stop @ OR Low - 0.4 ATR (keep V7)

If this works, we prove VWAP exit was the problem.
...
def run_orb_v13b_minimal(symbol, start, end):
    """
    V13b: V7 with VWAP loss exit removed (ONLY change)
    """
    ...
    params = {
        ...
        'VOL_MULT': 1.8,
        'PULLBACK_ATR': 0.15,
        'HARD_STOP_ATR': 0.4,  # V7 value
        'BREAKEVEN_TRIGGER_R': 0.5,  # V7 value
        'SCALE_13R_PCT': 0.50,  # V7 value
        'TRAIL_ATR': 0.6,  # V7 value
        'MIN_PRICE': 3.0,
        'ENABLE_VWAP_LOSS': False,  # ⭐ ONLY CHANGE FROM V7
    }
    
    print(f"\n{'='*60}")
    print(f"Testing {symbol} with ORB V13b MINIMAL")
    print(f"Period: {start} to {end}")
    print(f"CHANGE: VWAP loss exit DISABLED")
    print(f"{'='*60}")
    ...
    for i in range(len(df)):
        ...
        # Position management (same as V7 except VWAP exit)
            ...
            # ⭐ VWAP LOSS EXIT REMOVED (was in V7, now disabled)
