"""
ORB V22 "LET IT RUN" - Commodities Edition
Fix the win rate paradox: high win rate but losing money
Changes from V21:
- NO early scaling (let full position run)
- WIDER trailing stop (1.0 ATR instead of 0.6)
- SINGLE profit target at 2.0R (not 1.3R)
- Later breakeven (0.8R instead of 0.5R)
"""
import pandas as pd
import numpy as np
...
def run_orb_v22_let_it_run(symbol, start, end):
    """V22: Let winners run - fix the win rate paradox"""
    ...
    print(f"\nTesting {symbol} - V22 LET IT RUN ({session_hour}:{session_min:02d}) [{start} to {end}]")
    ...
    params = {
        ...
        'PULLBACK_ATR': 0.15,
        'HARD_STOP_ATR': 0.4,
        'BREAKEVEN_TRIGGER_R': 0.8,  # LATER breakeven (was 0.5)
        'PROFIT_TARGET_R': 2.0,       # SINGLE target far out (was 1.3 with scaling)
        'TRAIL_ATR': 1.0,             # WIDER trail (was 0.6)
        'MIN_PRICE': 0.01,
    ...
    for i in range(len(df)):
        ...
        if position is None:
            ...
            if breakout_seen:
                ...
                if (in_pullback_zone and 
                    ...
                    position = 1.0  # FULL POSITION (no scaling)
        ...
        # Position management
            ...
            # SINGLE profit target at 2.0R (NO early scaling)
