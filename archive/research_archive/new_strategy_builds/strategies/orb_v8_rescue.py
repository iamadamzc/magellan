"""
ORB V8 - RESCUE THE PARADOX
----------------------------
Two critical fixes based on data analysis:

1. LOWER TARGET: 1.3R → 0.7R (make winners reachable)
2. EARLIER ENTRY: Breakout, not pullback (catch momentum)
...
Expected: Flip paradox symbols (>50% win, negative P&L) to profitable
"""
...
def run_orb_v8(symbol, start, end):
    """ORB V8 - Lower target + Earlier entry"""
    ...
    params = {
        ...
        'SCALE_07R_PCT': 0.50,         # NEW: Scale at 0.7R instead of 1.3R
        'TRAIL_ATR': 0.6,
        'MIN_PRICE': 3.0,
    }
    ...
    for i in range(len(df)):
        ...
        # CHANGE 1: Enter on breakout (NO pullback requirement)
        if position is None:
            # Simple entry: breakout + volume + VWAP + price
        ...
        # Position management
            ...
            # CHANGE 2: Scale at 0.7R (instead of 1.3R)
    ...
    if len(trades) == 0:
        return {'symbol': symbol, 'total_trades': 0, 'win_rate': 0, 'avg_pnl': 0, 'total_pnl': 0, 'v7_pnl': 0, 'improvement': 0}
    ...
    return {
        ...
        'avg_pnl': avg_pnl,
        'total_pnl': total_pnl,
        'v7_pnl': 0,  # Will be filled in by test script
