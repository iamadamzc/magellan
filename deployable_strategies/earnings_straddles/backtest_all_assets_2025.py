...
IV_BY_TICKER = {
    'GOOGL': 0.25, 'AAPL': 0.22, 'AMD': 0.40, 'NVDA': 0.45, 'TSLA': 0.50,
    'MSFT': 0.23, 'AMZN': 0.28, 'META': 0.32, 'NFLX': 0.35, 'COIN': 0.60,
    'PLTR': 0.45
}
...
for ticker in sorted(tickers_to_test):
    ...
    for earnings_date_str in EARNINGS_2025[ticker]:
        ...
        print(f"  {earnings_date.date()} | ${entry_price:.2f} → ${exit_price:.2f} ({price_move_pct:+.1f}%) | "
    ...
    if len(ticker_trades) > 0:
        ...
        sharpe = (trade_returns.mean() / trade_returns.std() * np.sqrt(len(ticker_trades))) if trade_returns.std() > 0 else 0
        ...
        print(f"  {ticker} Summary: {len(ticker_trades)} trades | Win Rate: {win_rate:.1f}% | "
...
for ticker in sorted(results_by_ticker.keys()):
    ...
    print(f"{ticker:<8} | {r['trades']:6d} | {r['win_rate']:5.1f}% | ${r['avg_pnl']:>11,.0f} | "
...
if len(results_by_ticker) > 0:
    ...
    for ticker, r in ranked[:3]:
        if r['total_pnl'] > 0:
            print(f"  {ticker}: ${r['total_pnl']:+,.0f} total P&L | {r['win_rate']:.1f}% win rate | Sharpe {r['sharpe']:.2f}")
