...
print(f"Testing {sum(len(dates) for dates in EARNINGS_2024.values())} earnings events across {len(EARNINGS_2024)} tickers\n")
...
for ticker, earnings_dates in EARNINGS_2024.items():
    ...
    for earnings_date_str in earnings_dates:
        ...
        print(f"  {earnings_date.date()} | ${entry_price:.2f} → ${exit_price:.2f} ({price_move_pct:+.2f}%) | P&L: {pnl_pct:+.2f}% {win_symbol}")
...
for ticker in ['GOOGL', 'AAPL', 'AMD', 'NVDA', 'TSLA', 'MSFT', 'AMZN', 'META']:
    if ticker in results_by_ticker:
        ...
        print(f"{ticker:<8} | {r['trades']:6d} | {r['win_rate']:5.1f}% | {r['avg_pnl_pct']:7.2f}% | {r['sharpe']:6.2f} | {claimed:14.2f} | {status:>8}")
