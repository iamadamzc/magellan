...
for ticker in ['SPY', 'QQQ', 'IWM']:
    ...
    for event in FOMC_EVENTS_2024:
        ...
        print(f"  {event_date.date()} | ${entry_price:.2f} → ${exit_price:.2f} ({move_pct:+.2f}%) | P&L: {profit_pct:+.2f}% {win_symbol}")
...
for ticker in ['SPY', 'QQQ', 'IWM']:
    if ticker in results_by_ticker:
        ...
        print(f"{ticker:<8} | {r['trades']:6d} | {r['win_rate']:5.1f}% | {r['avg_pnl_pct']:7.2f}% | {r['sharpe']:6.2f} | {status:>8}")
