"""
ORB V23 "ALL DAY" - Remove first-hour restriction
Test if ORB setups work throughout the session, not just first hour
"""
...
def run_orb_v23_all_day(symbol, start, end):
    ...
    for i in range(len(df)):
        ...
        if position is None:
            ...
            if breakout_seen:
                ...
                if (
                    in_pullback_zone
                    and current_price > current_or_high
                    and current_price > current_vwap
                    and df.iloc[i]["volume_spike"] >= params["VOL_MULT"]
                    and current_price >= params["MIN_PRICE"]
                ):
        ...
        # Position management (same as V22)
