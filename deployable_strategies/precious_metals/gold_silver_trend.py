...
def load_precious_metal_data(symbol: str, interval: str = "1day", 
...
def run_trend_backtest(
    ...
    for i in range(len(df)):
        ...
        if position == 'flat' and rsi > entry_band:
            ...
            if verbose:
                print(f"  {date.strftime('%Y-%m-%d')} ENTER LONG @ ${price:.2f} (RSI: {rsi:.1f})")
        ...
        # Exit: RSI crosses below exit band
            ...
            if verbose:
                ...
                print(f"  {date.strftime('%Y-%m-%d')} EXIT @ ${price:.2f} | P&L: ${pnl:+,.2f} ({pnl_pct:+.1f}%) {win_loss}")
    ...
    if position == 'long':
        ...
        if verbose:
            win_loss = "✅" if pnl > 0 else "❌"
            print(f"  {final_date.strftime('%Y-%m-%d')} FINAL EXIT @ ${final_price:.2f} | P&L: ${pnl:+,.2f} ({pnl_pct:+.1f}%) {win_loss}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Precious Metals Trend Strategy Backtest")
    parser.add_argument("--symbol", default="GCUSD", choices=["GCUSD", "SIUSD"], help="Symbol to test")
