...
def run_backtest(df, rsi_period, entry_band, exit_band, leverage, transaction_cost_bps=5.0):
...
def run_optimization():
    ...
    print(f"Testing: {len(rsi_periods)} RSI periods × {len(entry_bands)} entry bands × {len(exit_bands)} exit bands × {len(leverages)} leverages")
    
    total_combos = len(rsi_periods) * len(entry_bands) * len(exit_bands) * len(leverages)
            ...
            for entry_band in entry_bands:
                for exit_band in exit_bands:
                    ...
                    for lev in leverages:
                        ...
                        result = run_backtest(df, rsi_period, entry_band, exit_band, lev)
                        ...
                        if result['meets_target'] and (best_target_match is None or lev < best_target_match['leverage']):
    ...
    for symbol in symbols:
        ...
        print(f"    RSI: {int(best_sharpe_row['rsi_period'])}, Entry: {int(best_sharpe_row['entry_band'])}, Exit: {int(best_sharpe_row['exit_band'])}")
        print(f"    Leverage: {int(best_sharpe_row['leverage'])}x | Daily: ${best_sharpe_row['avg_daily_pnl']:.0f}")
        
        if best_target_row is not None:
            print(f"\n  Lowest Leverage for $200/day:")
            print(f"    RSI: {int(best_target_row['rsi_period'])}, Entry: {int(best_target_row['entry_band'])}, Exit: {int(best_target_row['exit_band'])}")
            print(f"    Leverage: {int(best_target_row['leverage'])}x | Daily: ${best_target_row['avg_daily_pnl']:.0f}")
            print(f"    Sharpe: {best_target_row['sharpe']:.2f} | Max DD: {best_target_row['max_dd']:.0f}%")
