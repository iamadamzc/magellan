...
def identify_winning_events(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    print(
        f"Criteria: +{target_atr} ATR within {max_bars} bars, max DD {max_dd_atr} ATR"
    )
...
def discover_clusters(
    features: pd.DataFrame, events: pd.DataFrame, symbol: str
) -> dict:
    """
    ...
    print("-" * 70)
    print(
        f"{'Cluster':>8} {'Win Count':>10} {'Non Count':>10} {'Win Rate':>10} {'Lift':>10}"
    )
    print("-" * 70)
            ...
            hidden_states.append(
                {
                    ...
                    "centroid_raw": scaler.inverse_transform(
                        [kmeans.cluster_centers_[best_c]]
                    )[0],
                }
            )
...
def synthesize_strategy(cluster_results: dict, symbol: str) -> dict:
    ...
    for feat, val in sorted(
        zip(feat_cols, centroid_raw), key=lambda x: abs(x[1]), reverse=True
    )[:5]:
        direction = ">" if val > 0 else "<"
        print(f"  {feat} {direction} {val:.4f}")
        feature_profile.append(
            {"feature": feat, "threshold": float(val), "direction": direction}
        )
