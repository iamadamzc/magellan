...
with open(project_root / "test" / "vol_expansion" / "phase2_timestop_results.json") as f:
    phase2 = json.load(f)
...
for symbol in sorted(phase2.keys(), key=lambda s: phase2[s]['best_timestop']['return_pct'], reverse=True):
    p2 = phase2[symbol]
    ...
    print(f"{symbol:<8} {p2['cluster']:<8} {trend_str:<8} {rr_str:<12} {ts_str:<15} {best_ts['trades']:<8} {best_ts['win_rate']:<7.1%} {best_ts['expectancy']:<9.3f}R {best_ts['return_pct']:>8.1f}%")
...
for symbol in sorted(phase4.keys(), key=lambda s: phase4[s]['best_sniper']['return_pct'], reverse=True):
    best = phase4[symbol]['best_sniper']
    trend_str = "Yes" if best['trend_filter'] else "No"
    
    status = "✅ DEPLOY" if best['return_pct'] > 0 and best['trades'] >= 5 else "⚠️ LOW FREQ" if best['trades'] > 0 else "❌ SKIP"
    
    print(f"{symbol:<8} {best['percentile']:<6} {trend_str:<8} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}% {status:<10}")
...
total = sum(phase2[s]['best_timestop']['return_pct'] for s in phase2 if phase2[s]['best_timestop']['return_pct'] > 0)
