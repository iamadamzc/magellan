...
# Load final configs
with open(
    project_root / "test" / "vol_expansion" / "FINAL_OPTIMIZED_CONFIGS.json"
) as f:
    CONFIGS = json.load(f)
...
def backtest(
    df_test: pd.DataFrame, model: tuple, config: dict, track_equity: bool = False
) -> dict:
    ...
    if track_equity:
        equity_series = pd.Series(
            equity_curve, index=df_test.index[: len(equity_curve)]
        )
...
def walk_forward_validation(symbol: str, config: dict) -> dict:
    ...
    return {"validation_2024": val_result, "oos_2025": test_result}
...
def test_parameter_stability(symbol: str, config: dict) -> dict:
    ...
    for cluster_adj in [-1, 0, 1]:
        ...
        results.append(
            {"param": f"cluster_{test_cluster}", "return": result["return_pct"]}
        )
...
def main():
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            if result_2024:
                ...
                print(
                    f"    2024: {result_2024['trades']} trades, {result_2024['return_pct']:.1f}% return"
                )
        ...
        try:
            ...
            if wf_result:
                ...
                print(
                    f"    2024 (val): {wf_result['validation_2024']['return_pct']:.1f}%"
                )
    ...
    # Save results
    output_file = (
        project_root / "test" / "vol_expansion" / "comprehensive_validation.json"
    )
