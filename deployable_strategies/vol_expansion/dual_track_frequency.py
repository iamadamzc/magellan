...
def run_backtest(df: pd.DataFrame, signal_mask: pd.Series, 
                 target_atr: float, stop_atr: float, max_bars: int,
                 slippage: float) -> dict:
...
def run_track_a():
    ...
    for c in range(6):
        ...
        print(f"  Cluster {c}: WinRate={win_rate_c:.1%}, Lift={lift:.2f}x, Freq={signal_freq:.1%}, ~{trades_per_day:.1f} trades/day")
        ...
        backtest = run_backtest(df_5m, signal_mask, 
                               target_atr=2.0, stop_atr=1.0, max_bars=12,
                               slippage=0.02)
    ...
    viable = []
    for r in results:
        print(f"  Cluster {r['cluster']}: Trades={r['backtest_trades']:,}, HitRate={r['hit_rate']:.1%}, Expect={r['expectancy']:.3f}R, Target%={r['target_pct']:.1%}")
    ...
    if viable:
        ...
        print(f"    Best available: Cluster {best['cluster']} with {best['expectancy']:.3f}R")
...
def run_track_b():
    ...
    for c in range(8):
        ...
        # Backtest raw
        backtest_raw = run_backtest(df_15m, signal_mask_raw,
                                   target_atr=2.0, stop_atr=1.0, max_bars=8,
                                   slippage=0.02)
        
        # Backtest with trend filter
        backtest_trend = run_backtest(df_15m, signal_mask_trend,
                                     target_atr=2.0, stop_atr=1.0, max_bars=8,
                                     slippage=0.02)
    ...
    print("-"*70)
    print(f"{'Cluster':<8} {'Raw Trades':<12} {'Raw Exp.':<10} {'Trend Trades':<14} {'Trend Exp.':<12}")
    print("-"*70)
    
    workhorse_candidates = []
    
    for r in results:
        print(f"{r['cluster']:<8} {r['raw_trades']:<12} {r['raw_expectancy']:.3f}R{'':3} {r['trend_trades']:<14} {r['trend_expectancy']:.3f}R")
    ...
    if workhorse_candidates:
        ...
        for r in workhorse_candidates:
            print(f"    Cluster {r['cluster']}: Expect={r['trend_expectancy']:.3f}R, Freq={r['signal_freq']:.1%}, ~{r['trades_per_day']:.1f}/day")
        ...
        if high_freq:
            ...
            print(f"    Best high-freq: Cluster {best['cluster']} with {best['trend_expectancy']:.3f}R at {best['signal_freq']:.1%} freq")
        else:
...
def main():
    ...
    if best_5m['expectancy'] > 0.05 and best_5m['trades_per_day'] >= 1:
        candidates_viable.append(('5-MIN', best_5m['expectancy'], best_5m['trades_per_day']))
    
    if best_15m['trend_expectancy'] > 0.05 and best_15m['trades_per_day'] >= 1:
        candidates_viable.append(('15-MIN', best_15m['trend_expectancy'], best_15m['trades_per_day']))
