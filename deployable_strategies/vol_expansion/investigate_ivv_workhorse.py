...
def train_symbol_specific_model(df_train: pd.DataFrame, symbol: str) -> tuple:
    ...
    for c in range(8):
        ...
        print(f"  Cluster {c}: WinRate={win_rate:.1%}, Lift={lift:.2f}x, Freq={freq:.1%}")
...
def main():
    ...
    for r in sorted(results, key=lambda x: x['return_pct'], reverse=True):
        ...
        print(f"{r['cluster']:<8} {trend_str:<8} {r['trades']:<8} {r['win_rate']:<7.1%} {r['expectancy']:<9.3f}R {r['return_pct']:>8.1f}%")
    ...
    print(f"{'Win Rate':<20} {best['win_rate']:<19.1%} {cluster_7_trend['win_rate']:<19.1%}")
    print(f"{'Expectancy':<20} {best['expectancy']:<19.3f}R {cluster_7_trend['expectancy']:<19.3f}R")
    print(f"{'Return':<20} {best['return_pct']:<19.1f}% {cluster_7_trend['return_pct']:<19.1f}%")
