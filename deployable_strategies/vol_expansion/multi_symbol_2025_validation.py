...
def run_symbol_backtest(
    symbol: str, df_test: pd.DataFrame, workhorse_model: tuple
) -> dict:
    ...
    def calc_stats(trades, account):
        ...
        df_t = pd.DataFrame(
            [
                {"pnl": t.pnl_dollars, "pnl_r": t.pnl_r, "is_win": t.is_win}
                for t in trades
            ]
        )
...
def main():
    ...
    for symbol in symbols:
        ...
        try:
            ...
            df_test = df_15m[
                (df_15m.index >= "2025-01-01") & (df_15m.index < "2026-01-01")
            ]
            ...
            print(f"\n  RESULTS:")
            print(
                f"    Sniper:    {results['sniper']['trades']:3d} trades, {results['sniper']['return_pct']:6.1f}% return"
            )
            print(
                f"    Workhorse: {results['workhorse']['trades']:3d} trades, {results['workhorse']['return_pct']:6.1f}% return"
            )
            print(
                f"    Combined:  {results['combined']['trades']:3d} trades, {results['combined']['return_pct']:6.1f}% return"
            )
    ...
    print(
        f"{'':8} {'Trades':>6} {'Return':>8} {'Expect':>8} {'Trades':>6} {'Return':>8} {'Expect':>8} {'Return':>10} {'Total':>8}"
    )
    print("-" * 100)
    ...
    # Save results
    output_file = (
        project_root / "test" / "vol_expansion" / "multi_symbol_2025_results.json"
    )
