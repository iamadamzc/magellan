...
def main():
    ...
    print(
        f"{'Final:':<20} ${results['sniper_final']:>8,.2f}{'':>7} ${results['workhorse_final']:>8,.2f}"
    )
    print(f"{'P&L:':<20} ${s_pnl:>8,.2f}{'':>7} ${w_pnl:>8,.2f}")
    print(
        f"{'Return:':<20} {(results['sniper_final']/25000-1)*100:>7.2f}%{'':>8} {(results['workhorse_final']/25000-1)*100:>7.2f}%"
    )
    print()
    ...
    if len(results["daily_pnl"]) > 0:
        ...
        monthly = daily.groupby("month").agg(
            {"sniper_pnl": "sum", "workhorse_pnl": "sum"}
        )
        ...
        for month, row in monthly.iterrows():
            ...
            print(
                f"{str(month):<15} ${row['sniper_pnl']:>8,.2f}{'':>4} ${row['workhorse_pnl']:>8,.2f}{'':>4} ${combined_month:>8,.2f}"
            )
