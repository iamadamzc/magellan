...
def backtest_config(
    df_test: pd.DataFrame, model: tuple, cluster_id: int, use_trend: bool
) -> dict:
...
def optimize_symbol(symbol: str) -> dict:
    ...
    for cluster_id in range(8):
        for use_trend in [False, True]:
            ...
            results.append({"cluster": cluster_id, "trend_filter": use_trend, **result})
    ...
    return {"symbol": symbol, "best": best, "all_results": results}
...
def main():
    ...
    for symbol, data in sorted(
        all_optimizations.items(),
        key=lambda x: x[1]["best"]["return_pct"],
        reverse=True,
    ):
        best = data["best"]
        trend_str = "Yes" if best["trend_filter"] else "No"
        print(
            f"{symbol:<8} {best['cluster']:<8} {trend_str:<8} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%"
        )
    ...
    profitable = {
        s: d for s, d in all_optimizations.items() if d["best"]["return_pct"] > 0
    }
    print(f"\n✅ Profitable symbols: {len(profitable)}/{len(all_optimizations)}")
    for symbol in sorted(
        profitable.keys(),
        key=lambda s: profitable[s]["best"]["return_pct"],
        reverse=True,
    ):
    ...
    if unprofitable:
        ...
        for symbol in sorted(
            unprofitable.keys(), key=lambda s: unprofitable[s]["best"]["return_pct"]
        ):
