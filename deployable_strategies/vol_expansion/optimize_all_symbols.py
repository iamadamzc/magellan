"""
COMPREHENSIVE SYMBOL OPTIMIZATION SUITE

For each symbol:
1. Train symbol-specific model
2. Test all 8 clusters
3. Test with/without trend filter
...
def backtest_config(df_test: pd.DataFrame, model: tuple, cluster_id: int, use_trend: bool) -> dict:
    """Backtest a specific cluster/filter configuration."""
...
def optimize_symbol(symbol: str) -> dict:
    """Find optimal configuration for a symbol."""
    ...
    if len(df_test) < 100:
        print(f"  ⚠️ Insufficient 2025 data")
        return None
    
    print(f"  Training: {len(df_train):,} bars | Testing: {len(df_test):,} bars")
    ...
    for cluster_id in range(8):
        for use_trend in [False, True]:
            result = backtest_config(df_test, model, cluster_id, use_trend)
            results.append({
                'cluster': cluster_id,
...
def main():
    print("="*70)
    print("COMPREHENSIVE SYMBOL OPTIMIZATION SUITE")
    print("Testing all symbols × all clusters × all filters")
    print("="*70)
    ...
    for symbol in symbols:
        try:
            ...
            if result:
                all_optimizations[symbol] = result
    ...
    print(f"\n{'Symbol':<8} {'Cluster':<8} {'Trend?':<8} {'Trades':<8} {'Win%':<8} {'Expect':<10} {'Return':<10}")
    print("-"*70)
    
    for symbol, data in sorted(all_optimizations.items(), key=lambda x: x[1]['best']['return_pct'], reverse=True):
        best = data['best']
        trend_str = "Yes" if best['trend_filter'] else "No"
        print(f"{symbol:<8} {best['cluster']:<8} {trend_str:<8} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%")
    
    # Save detailed results
    output = {}
    for symbol, data in all_optimizations.items():
        output[symbol] = {
            'best_config': data['best'],
            'original_config': {
                'cluster': 7,
                'trend_filter': True,
                'note': 'SPY-trained defaults'
            }
        }
    
    output_file = project_root / "test" / "vol_expansion" / "optimized_configs.json"
    ...
    profitable = {s: d for s, d in all_optimizations.items() if d['best']['return_pct'] > 0}
    print(f"\n✅ Profitable symbols: {len(profitable)}/{len(all_optimizations)}")
    for symbol in sorted(profitable.keys(), key=lambda s: profitable[s]['best']['return_pct'], reverse=True):
        ret = profitable[symbol]['best']['return_pct']
        print(f"  {symbol}: {ret:>6.1f}%")
    
    unprofitable = {s: d for s, d in all_optimizations.items() if d['best']['return_pct'] <= 0}
    if unprofitable:
        print(f"\n❌ Still unprofitable: {len(unprofitable)}/{len(all_optimizations)}")
        for symbol in sorted(unprofitable.keys(), key=lambda s: unprofitable[s]['best']['return_pct']):
            ret = unprofitable[symbol]['best']['return_pct']
            print(f"  {symbol}: {ret:>6.1f}%")
