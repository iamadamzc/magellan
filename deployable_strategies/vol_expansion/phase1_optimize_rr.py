...
def main():
    ...
    for symbol in OPTIMIZED_CONFIGS.keys():
        ...
        for rr in RR_CONFIGS:
            ...
            print(
                f"    {rr['name']}: {result['trades']} trades, {result['return_pct']:.1f}%"
            )
    ...
    for symbol, data in sorted(
        all_results.items(), key=lambda x: x[1]["best_rr"]["return_pct"], reverse=True
    ):
        best = data["best_rr"]
        print(
            f"{symbol:<8} {best['config']:<18} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%"
        )
