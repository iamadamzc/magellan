...
Test different target/stop combinations for each symbol using their
already-optimized cluster and trend settings.
...
# Optimized configs from Phase 0
OPTIMIZED_CONFIGS = {
    'SPY': {'cluster': 6, 'trend_filter': False},
    'QQQ': {'cluster': 4, 'trend_filter': False},
    'IWM': {'cluster': 1, 'trend_filter': False},
    'IVV': {'cluster': 0, 'trend_filter': False},
    'VOO': {'cluster': 4, 'trend_filter': False},
    'GLD': {'cluster': 4, 'trend_filter': False},
    'SLV': {'cluster': 6, 'trend_filter': False},
    'TQQQ': {'cluster': 2, 'trend_filter': True},
    'SOXL': {'cluster': 1, 'trend_filter': True},
}

# R:R combinations to test
RR_CONFIGS = [
    {'target': 1.5, 'stop': 1.0, 'name': '1.5:1'},
    {'target': 2.0, 'stop': 1.0, 'name': '2:1 (baseline)'},
    {'target': 2.5, 'stop': 1.0, 'name': '2.5:1'},
    {'target': 3.0, 'stop': 1.0, 'name': '3:1'},
    {'target': 2.0, 'stop': 0.5, 'name': '4:1 (tight stop)'},
    {'target': 2.0, 'stop': 1.5, 'name': '1.33:1 (wide stop)'},
]
...
def backtest_rr(df_test: pd.DataFrame, model: tuple, cluster_id: int, use_trend: bool,
                target_mult: float, stop_mult: float) -> dict:
...
def main():
    ...
    for symbol in OPTIMIZED_CONFIGS.keys():
        print(f"\n{'='*70}")
        print(f"OPTIMIZING R:R FOR: {symbol}")
        print(f"{'='*70}")
        
        config = OPTIMIZED_CONFIGS[symbol]
        ...
        print(f"  Testing R:R combinations...")
        symbol_results = []
        
        for rr in RR_CONFIGS:
            result = backtest_rr(df_test, model, config['cluster'], config['trend_filter'],
                                 rr['target'], rr['stop'])
            result['config'] = rr['name']
            result['target'] = rr['target']
            result['stop'] = rr['stop']
            symbol_results.append(result)
            print(f"    {rr['name']}: {result['trades']} trades, {result['return_pct']:.1f}%")
        
        best = max(symbol_results, key=lambda x: x['return_pct'])
        all_results[symbol] = {
            'best_rr': best,
            'all_rr': symbol_results,
    ...
    # Summary
    print("\n" + "="*70)
    print("R:R OPTIMIZATION SUMMARY")
    print("="*70)
    
    print(f"\n{'Symbol':<8} {'Best R:R':<18} {'Trades':<8} {'Win%':<8} {'Expect':<10} {'Return':<10}")
    print("-"*70)
    
    for symbol, data in sorted(all_results.items(), key=lambda x: x[1]['best_rr']['return_pct'], reverse=True):
        best = data['best_rr']
        print(f"{symbol:<8} {best['config']:<18} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%")
    
    # Save
    output_file = project_root / "test" / "vol_expansion" / "phase1_rr_results.json"
