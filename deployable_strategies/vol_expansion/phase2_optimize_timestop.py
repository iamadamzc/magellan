...
def backtest_timestop(df_test: pd.DataFrame, model: tuple, cluster_id: int, use_trend: bool,
                      target_mult: float, stop_mult: float, max_hold_bars: int) -> dict:
...
def main():
    ...
    for symbol, p1_data in PHASE1_RESULTS.items():
        ...
        for ts in TIME_STOPS:
            result = backtest_timestop(df_test, model, cluster, trend_filter,
                                       best_rr['target'], best_rr['stop'], ts['bars'])
            result['config'] = ts['name']
            result['bars'] = ts['bars']
            symbol_results.append(result)
            print(f"    {ts['name']}: {result['trades']} trades, {result['return_pct']:.1f}%")
    ...
    for symbol, data in sorted(all_results.items(), key=lambda x: x[1]['best_timestop']['return_pct'], reverse=True):
        best = data['best_timestop']
        print(f"{symbol:<8} {best['config']:<20} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%")
    
    # Save
    output_file = project_root / "test" / "vol_expansion" / "phase2_timestop_results.json"
