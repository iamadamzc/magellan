...
def derive_sniper_thresholds(df_train: pd.DataFrame, percentile: float = 10) -> dict:
    ...
    thresholds = {
        'effort_result_mean': float(np.percentile(winners['effort_result_mean'].dropna(), percentile)),
        'range_ratio_mean': float(np.percentile(winners['range_ratio_mean'].dropna(), 100 - percentile)),
        'volatility_ratio_mean': float(np.percentile(winners['volatility_ratio_mean'].dropna(), 100 - percentile))
    }
    
    return thresholds


def backtest_sniper(df_test: pd.DataFrame, thresholds: dict, use_trend: bool,
                    target_mult: float = 2.0, stop_mult: float = 1.0, max_hold_bars: int = 8) -> dict:
...
def main():
    ...
    for symbol in symbols:
        ...
        for pct in percentiles:
            ...
            for use_trend in [False, True]:
                ...
                print(f"    {trend_str}: {result['trades']} trades, {result['return_pct']:.1f}%")
        
        if symbol_results:
            best = max(symbol_results, key=lambda x: x['return_pct'])
            all_results[symbol] = {
                'best_sniper': best,
                'all_configs': symbol_results
            }
    ...
    for symbol, data in sorted(all_results.items(), key=lambda x: x[1]['best_sniper']['return_pct'], reverse=True):
        best = data['best_sniper']
        trend_str = "Yes" if best['trend_filter'] else "No"
        print(f"{symbol:<8} {best['percentile']:<6} {trend_str:<8} {best['trades']:<8} {best['win_rate']:<7.1%} {best['expectancy']:<9.3f}R {best['return_pct']:>8.1f}%")
