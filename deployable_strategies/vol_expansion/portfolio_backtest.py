...
def train_workhorse_model(features: pd.DataFrame, df: pd.DataFrame) -> tuple:
    ...
    print(f"  Trained on {len(X_all):,} samples ({n_sample:,} wins + {n_sample:,} non-wins)")
    ...
    for idx in range(len(df)):
        ...
        if sniper_position is None:
            ...
            if exit_signal:
                ...
                pnl_gross = (raw_exit - sniper_position['raw_entry']) * sniper_position['shares']
        ...
        if workhorse_position is None:
            ...
            if exit_signal:
                exit_price = raw_exit - slippage/2
                pnl_gross = (raw_exit - workhorse_position['raw_entry']) * workhorse_position['shares']
...
def analyze_results(results: Dict):
    ...
    if sniper_trades:
        ...
        sniper_total = sniper_wins = sniper_hr = sniper_total_pnl = sniper_avg_pnl = sniper_expect = 0
    ...
    if workhorse_trades:
        ...
        workhorse_total = workhorse_wins = workhorse_hr = workhorse_total_pnl = workhorse_avg_pnl = workhorse_expect = 0
    
    # Print results
    print(f"\n{'SNIPER (Dip Buyer)':^35} {'WORKHORSE (Cluster 7)':^35}")
    print("-"*70)
    print(f"{'Starting Capital:':<25} $25,000 {'':>10} $25,000")
    print(f"{'Final Balance:':<25} ${results['sniper_final']:>8,.2f} {'':>8} ${results['workhorse_final']:>8,.2f}")
    print(f"{'Total P&L:':<25} ${sniper_total_pnl:>8,.2f} {'':>8} ${workhorse_total_pnl:>8,.2f}")
    print(f"{'Return:':<25} {(results['sniper_final']/25000-1)*100:>7.2f}% {'':>8} {(results['workhorse_final']/25000-1)*100:>7.2f}%")
    print()
    print(f"{'Total Trades:':<25} {sniper_total:>10} {'':>10} {workhorse_total:>10}")
    print(f"{'Win Rate:':<25} {sniper_hr:>9.1%} {'':>10} {workhorse_hr:>9.1%}")
    print(f"{'Avg P&L/Trade:':<25} ${sniper_avg_pnl:>8.2f} {'':>8} ${workhorse_avg_pnl:>8.2f}")
    print(f"{'Expectancy (R):':<25} {sniper_expect:>9.3f}R {'':>7} {workhorse_expect:>9.3f}R")
    ...
    if len(daily_pnl) > 0:
        ...
        for _, row in daily_pnl.tail(10).iterrows():
            ...
            print(f"{str(row['date']):<12} ${row['sniper_pnl']:>11,.2f} ${row['workhorse_pnl']:>14,.2f} ${combined_day:>11,.2f} ${total_balance:>14,.2f}")
        ...
        for week, row in weekly.tail(8).iterrows():
            ...
            print(f"{str(week):<15} ${row['sniper_pnl']:>11,.2f} ${row['workhorse_pnl']:>14,.2f} ${combined_week:>11,.2f}")
...
def main():
    ...
    # Save results
    output_file = project_root / "test" / "vol_expansion" / "portfolio_backtest_results.json"
    ...
    results_summary = {
        ...
        'combined': {
            ...
            'return_pct': ((results['sniper_final'] + results['workhorse_final']) / 50000 - 1) * 100,
            'total_trades': len(results['sniper_trades']) + len(results['workhorse_trades'])
        }
    }
