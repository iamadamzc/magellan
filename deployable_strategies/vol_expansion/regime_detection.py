...
def find_best_windows(df_intraday: pd.DataFrame, symbol: str, window_days: int, top_pct: float = 0.2):
    """Find best performing windows of specified length."""
    
    # Get daily data
    daily = df_intraday.resample('1D').agg({
        'close': 'last'
    }).dropna()
    ...
    print(f"  {window_days}-day windows: Found {len(best_windows)} in top {top_pct*100}%")
...
def match_current_regime(current_features: dict, signature: dict, tolerance: float = 0.3) -> bool:
...
def main():
    ...
    for date in regime_features.index:
        ...
        regime_matches.append({
            'date': date,
            'regime': matched_window if matched_window else 'NO_TRADE'
        })
