...
def main():
    ...
    logger.info(f"  Entry: ER_zscore < {config['entry_conditions']['effort_result_zscore_max']}, "
               f"RR > {config['entry_conditions']['range_ratio_mean_min']}, "
               f"VR > {config['entry_conditions']['volatility_ratio_min']}")
    logger.info(f"  Exit: Target={config['exit_rules']['target_atr_multiple']}×ATR, "
            ...
            if time.time() - last_health_check > health_check_interval:
                ...
                for regime, stats in status.get('regime_stats', {}).items():
                    if stats['trades'] > 0:
                        ...
                        logger.info(f"  {regime}: {stats['trades']} trades, {hit_rate:.1%} win rate")
