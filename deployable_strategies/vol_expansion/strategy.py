...
class VolatilityExpansionStrategy:
    ...
    def __init__(
        ...
        self.logger.info(
            f"✓ VolatilityExpansionStrategy v2.0 initialized for {len(symbols)} symbols"
        )
        self.logger.info(
            f"  Entry thresholds: ER_zscore < {self.entry_config.get('effort_result_zscore_max', -0.5)}, "
            ...
            [
                {
                    ...
                    "volume": bar.volume,
                    "trade_count": getattr(
                        bar, "trade_count", bar.volume // 100
                    ),  # Fallback
                }
                for bar in bars
            ]
    ...
    def _evaluate_entry(self, symbol: str, current, df):
        ...
        risk_dollars = float(account.equity) * self.params.get(
        ...
        if shares > 0:
            self._enter_position(
                symbol, shares, current, stop_loss, target_price, atr, df
            )
    ...
    def _enter_position(
        ...
        try:
            ...
            regime = self._estimate_vix_regime(
                current.get("volatility_ratio_mean", 1.0)
            )
            ...
            self.trade_logger.log_trade(
                ...
                risk_metrics={
                    ...
                    "risk_per_share": current["close"] - stop_loss,
                    "position_dollars": shares * current["close"],
                    "risk_reward_ratio": (target - current["close"])
                    / (current["close"] - stop_loss),
                },
            )
    ...
    def _exit_position(self, symbol: str, exit_price: float, reason: str, current):
        ...
        try:
            ...
            self.trade_logger.log_trade(
                ...
                additional_metrics={
                    "pnl_r": pnl_r,
                    "max_favorable_excursion": pos["highest_price"]
                    - pos["entry_price"],
                },
            )
