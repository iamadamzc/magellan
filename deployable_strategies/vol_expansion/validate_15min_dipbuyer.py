...
class DipBuyerValidator:
    ...
    def check_entry_signal(
        self, row: pd.Series, config: BacktestConfig, thresholds: dict
    ) -> bool:
    ...
    def run_backtest(
        self, df: pd.DataFrame, config: BacktestConfig, symbol: str
    ) -> List[Trade]:
    ...
    def analyze_results(
        self, trades: List[Trade], config: BacktestConfig, total_bars: int
    ) -> Dict:
        ...
        for reason in ["TARGET", "STOP", "TIME"]:
            ...
            reason_hr = (
                df[df["exit_reason"] == reason]["is_win"].mean() if count > 0 else 0
            )
    ...
    def print_results(self, metrics: Dict):
        ...
        for reason, data in metrics["exit_breakdown"].items():
            print(
                f"  {reason:8s}: {data['count']:5d} | Win Rate: {data['win_rate']:.1%}"
            )
...
def main():
    ...
    for m in all_results:
        ...
        print(
            f"{name:<35} {m['total_trades']:<10,} {m['hit_rate']:.1%}{'':3} {m['expectancy_r']:.3f}R{'':4} {m['trades_per_week']:.0f}"
        )
