...
class VolExpansionBacktestV2:
    ...
    def __init__(self, config_path: Path):
        ...
        for symbol, data in self.research_thresholds.items():
            print(f"  {symbol}: Signal Freq={data['signal_frequency']:.1%}, Hit Rate={data['hit_rate']:.1%}")
    ...
    def _load_vix_data(self):
        ...
        try:
            ...
            vix_files = list(vix_path.glob("*VIX*1min*.parquet")) + list(vix_path.glob("*VIX*1d*.parquet"))
            ...
            spy_files = sorted((project_root / "data" / "cache" / "equities").glob("SPY_1d_*.parquet"))
            if spy_files:
    ...
    def _align_vix(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['vix'] = df['date'].map(lambda d: vix_daily.loc[pd.Timestamp(d), 'vix'] if pd.Timestamp(d) in vix_daily.index else None)
    ...
    def analyze_results(self, trades: list, symbol: str):
        ...
        for regime in ['COMPLACENCY', 'NORMAL', 'PANIC']:
            ...
            if len(regime_trades) > 0:
                ...
                print(f"{regime:15s}: {len(regime_trades):3d} trades | "
        ...
        print(f"Actual Hit Rate:     {hit_rate:.1%} ({hit_rate_diff:+.1f}pp) {hit_rate_status}")
...
def main():
    ...
    # Test periods
    test_periods = {
        'FULL': {'start': '2022-01-01', 'end': '2026-01-24', 'description': 'Full Dataset (In-Sample)'},
        'OOS': {'start': '2025-01-01', 'end': '2026-01-24', 'description': 'Out-of-Sample (Walk-Forward)'}
    }
    ...
    for period_name, period_results in all_results.items():
        ...
        for symbol, results in period_results.items():
            print(f"  {symbol}: Hit Rate={results['hit_rate']:.1%}, "
