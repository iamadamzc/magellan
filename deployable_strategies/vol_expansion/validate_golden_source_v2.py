...
class GoldenSourceValidatorV2:
    ...
    def merge_and_scale(self, features: pd.DataFrame, prices: pd.DataFrame) -> tuple:
        ...
        scaled_features = pd.DataFrame(
            X_scaled, index=merged.index, columns=feature_cols
        )
...
def main():
    ...
    for symbol in ["SPY"]:
        try:
            ...
            # Merge AND SCALE
            merged, scaled_features, feature_cols, scaler = validator.merge_and_scale(
                features, prices
            )

            # Run backtest with scaled features
            trades = validator.run_backtest(
                symbol, merged, scaled_features, feature_cols
            )
