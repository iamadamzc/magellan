...
class SniperStressTest:
    ...
    def prepare_data(self, features: pd.DataFrame, prices: pd.DataFrame,
    ...
    def run_backtest(self, merged: pd.DataFrame, config: BacktestConfig) -> List[TradeResult]:
    ...
    def calculate_metrics(self, trades: List[TradeResult], config: BacktestConfig) -> Dict:
        ...
        if df['pnl_r'].std() > 0:
            sharpe_proxy = (df['pnl_r'].mean() / df['pnl_r'].std()) * np.sqrt(252 * 6.5 * 60 / 30)  # Annualized
        else:
            sharpe_proxy = 0
        ...
        for reason in ['TARGET_HIT', 'STOP_LOSS', 'TIME_STOP']:
            ...
            exit_breakdown[reason] = {'count': count, 'pct': count/total_trades*100, 'win_rate': reason_hr}
        
        # P&L per trade stats
        avg_pnl_dollars = df['pnl_net'].mean()
        avg_slippage = df['slippage_cost'].mean()
        slippage_pct_of_pnl = abs(avg_slippage / avg_pnl_dollars * 100) if avg_pnl_dollars != 0 else float('inf')
    ...
    def print_results(self, metrics: Dict, config: BacktestConfig):
        ...
        print(f"Stop: {config.stop_atr} ATR | Target: {config.target_atr} ATR | R:R = 1:{config.target_atr/config.stop_atr:.1f}")
        ...
        for reason, data in metrics['exit_breakdown'].items():
            print(f"  {reason:12s}: {data['count']:5d} ({data['pct']:5.1f}%) | Win Rate: {data['win_rate']:.1%}")
        
        # Psychological viability assessment
        print(f"\n--- PSYCHOLOGICAL VIABILITY ---")
        if metrics['max_consecutive_losses'] <= 10:
            print(f"  Losing Streak: MANAGEABLE ({metrics['max_consecutive_losses']} max)")
        elif metrics['max_consecutive_losses'] <= 15:
            print(f"  Losing Streak: CHALLENGING ({metrics['max_consecutive_losses']} max)")
        else:
            print(f"  Losing Streak: EXTREME ({metrics['max_consecutive_losses']} max) - May cause abandonment!")
        
        if metrics['slippage_pct_of_pnl'] < 20:
            print(f"  Slippage Impact: LOW ({metrics['slippage_pct_of_pnl']:.1f}% of profits)")
        elif metrics['slippage_pct_of_pnl'] < 50:
            print(f"  Slippage Impact: MODERATE ({metrics['slippage_pct_of_pnl']:.1f}% of profits)")
        else:
            print(f"  Slippage Impact: SEVERE ({metrics['slippage_pct_of_pnl']:.1f}% of profits) - Strategy may be fragile!")
...
def main():
    ...
    merged = tester.prepare_data(features, prices, scaler, kmeans, feat_cols, best_cluster)
    ...
    print(f"{'Max Consec. Losses':<25} {m_a['max_consecutive_losses']:<20} {m_b['max_consecutive_losses']:<20}")
    ...
    if m_a['expectancy_r'] > 0 or m_b['expectancy_r'] > 0:
        ...
        print(f"3. Prepare for up to {best['max_consecutive_losses']} consecutive losses")
