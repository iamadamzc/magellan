...
def load_gold_data(start_date: str = None, end_date: str = None) -> pd.DataFrame:
    """Load gold daily data from cache."""
    cache_dir = project_root / "data" / "cache" / "futures"
    pattern = "GCUSD_1day_*.parquet"
    files = list(cache_dir.glob(pattern))
    
    if not files:
        raise FileNotFoundError(f"No GCUSD data found in {cache_dir}")
...
def calculate_rsi(prices: pd.Series, period: int = 35) -> pd.Series:
...
def run_gold_backtest(
    ...
    """Run gold trend-following backtest."""
    
    if verbose:
        print("\n" + "=" * 60)
        print("GOLD TREND-FOLLOWING BACKTEST")
        print("=" * 60)
    ...
    # Load data
    df = load_gold_data(start_date, end_date)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gold Trend Strategy Backtest")
