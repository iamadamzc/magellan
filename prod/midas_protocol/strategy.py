...
class MIDASProtocolStrategy:
    ...
    def __init__(
        ...
        # Entry parameters
        self.glitch_velocity = config["entry_logic"]["glitch_guard"]["velocity_threshold"]
        
        # Setup A (Crash Reversal)
        self.setup_a_vel_min = config["entry_logic"]["setup_a_crash_reversal"]["velocity_min"]
        self.setup_a_vel_max = config["entry_logic"]["setup_a_crash_reversal"]["velocity_max"]
        self.setup_a_ema_dist = config["entry_logic"]["setup_a_crash_reversal"]["ema_distance_max"]
        self.setup_a_atr_min = config["entry_logic"]["setup_a_crash_reversal"]["atr_ratio_min"]
        
        # Setup B (Quiet Drift)
        self.setup_b_vel_max = config["entry_logic"]["setup_b_quiet_drift"]["velocity_max"]
        self.setup_b_ema_dist = config["entry_logic"]["setup_b_quiet_drift"]["ema_distance_max"]
        self.setup_b_atr_min = config["entry_logic"]["setup_b_quiet_drift"]["atr_ratio_min"]
        self.setup_b_atr_max = config["entry_logic"]["setup_b_quiet_drift"]["atr_ratio_max"]
    ...
    def evaluate_entry(self, symbol: str) -> Optional[str]:
        ...
        if velocity < self.glitch_velocity:
            self.logger.warning(f"GLITCH GUARD TRIGGERED: Velocity={velocity:.2f} < {self.glitch_velocity}")
            return None
        ...
        # ATR Ratio > 0.50
        if (self.setup_a_vel_min <= velocity <= self.setup_a_vel_max and
            ema_distance <= self.setup_a_ema_dist and
            atr_ratio > self.setup_a_atr_min):
            
            self.logger.info(f"SETUP A (Crash Reversal) TRIGGERED:")
            self.logger.info(f"  Velocity: {velocity:.2f} (range: {self.setup_a_vel_min} to {self.setup_a_vel_max})")
            self.logger.info(f"  EMA Distance: {ema_distance:.2f} (<= {self.setup_a_ema_dist})")
        ...
        if (velocity <= self.setup_b_vel_max and
            ema_distance <= self.setup_b_ema_dist and
            self.setup_b_atr_min <= atr_ratio <= self.setup_b_atr_max):
            
            self.logger.info(f"SETUP B (Quiet Drift) TRIGGERED:")
            self.logger.info(f"  Velocity: {velocity:.2f} (<= {self.setup_b_vel_max})")
            self.logger.info(f"  EMA Distance: {ema_distance:.2f} (<= {self.setup_b_ema_dist})")
            self.logger.info(f"  ATR Ratio: {atr_ratio:.4f} (range: {self.setup_b_atr_min} to {self.setup_b_atr_max})")
            return 'setup_b'
    ...
    def enter_position(self, symbol: str, setup: str):
        ...
        try:
            ...
            self.logger.info(f"  Stop Loss: {stop_loss:.2f} (-{self.stop_loss_points} pts / $40)")
            self.logger.info(f"  Take Profit: {take_profit:.2f} (+{self.take_profit_points} pts / $240)")
