...
def run_bear_trap(symbol, start, end, initial_capital=100000):
    ...
    df['session_high'] = df.groupby('date')['high'].transform('max')
    ...
    for i in range(len(df)):
        ...
        if position is None:
            ...
            is_reclaim = (
                df.iloc[i]['close'] > session_low and  # Reclaimed above session low
                df.iloc[i]['wick_ratio'] >= params['RECLAIM_WICK_RATIO_MIN'] and
                df.iloc[i]['body_ratio'] >= params['RECLAIM_BODY_RATIO_MIN'] and
                df.iloc[i]['volume_ratio'] >= (1 + params['RECLAIM_VOL_MULT'])
            )
