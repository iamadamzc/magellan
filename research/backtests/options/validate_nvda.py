"""
Premium Selling Validation - NVDA (2025 only, avoiding June 2024 split)

Testing if higher volatility = higher returns
"""
...
print("="*70)
print("PREMIUM SELLING - NVDA VALIDATION (2025)")
print("="*70)
print("Testing high-volatility stock (NVDA ~3-5% daily vol vs SPY ~0.8%)\n")

# Fetch NVDA 2025 data (avoid June 2024 split)
...
r = 0.04
sigma = 0.40  # NVDA has ~2x volatility of SPY (0.40 vs 0.20)
...
nvda_return = (df['close'].iloc[-1] / df['close'].iloc[0] - 1) * 100

print("="*70)
print("NVDA RESULTS (2025)")
print("="*70)
print(f"\n📊 Performance:")
...
print("COMPARISON")
print("="*70)
print(f"\nSPY 2025:  +575.84% return, 10 trades, 60.0% win rate")
print(f"NVDA 2025: {total_return:+.2f}% return, {len(trades_df)} trades, {win_rate:.1f}% win rate")
print("\n✅ Higher volatility = " + ("BETTER" if total_return > 575 else "SIMILAR" if total_return > 400 else "WORSE") + " returns")
print("="*70)

# Save
output_dir = Path('results/options')
output_dir.mkdir(parents=True, exist_ok=True)
trades_df.to_csv(output_dir / 'nvda_premium_selling_trades.csv', index=False)
