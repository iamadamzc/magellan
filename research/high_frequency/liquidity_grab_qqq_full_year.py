"""
Liquidity Grab QQQ - Full Year Validation (2024 + 2025)

Testing baseline config that achieved Sharpe 0.84 on Q1 2024:
- Lookback: 10
- Rejection bars: 3
- Profit target: 0.375%
- Stop loss: 0.15%
...
def liquidity_grab_qqq(df):
    """Best QQQ config from Q1 testing"""
    lookback = 10
    rejection_bars = 3
    profit_target = 0.375
    stop_loss = 0.15
...
print("="*80)
print("LIQUIDITY GRAB QQQ - FULL YEAR VALIDATION")
print("="*80)
print("\nConfig: 0.375% target, 0.15% stop, 15min hold, 1.0 bps friction")
...
if '2024' in results_by_year and '2025' in results_by_year:
    ...
    if avg_sharpe > 1.0:
        ...
        print(f"   Strategy is profitable and ready for deployment")
    elif avg_sharpe > 0.5:
        print(f"\n✅ **PROFITABLE** Avg Sharpe {avg_sharpe:.2f}")
        print(f"   Consider further optimization or deploy with caution")
    else:
        print(f"\n⚠️  Avg Sharpe {avg_sharpe:.2f} - marginal profitability")

# Save
with open('liquidity_grab_qqq_full_year_results.json', 'w') as f:
    json.dump(results_by_year, f, indent=2)

print(f"\n✅ Saved to liquidity_grab_qqq_full_year_results.json")
