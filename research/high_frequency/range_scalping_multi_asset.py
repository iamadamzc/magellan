"""
Range Scalping - Multi-Asset Class Testing

Strategy Concept:
- Identify when price is range-bound (consolidating)
- Buy at support, sell at resistance
- Exit when range breaks or target/stop hit

Entry signals:
1. Price touches lower Bollinger Band → Long
2. Price touches upper Bollinger Band → Short
3. Confirm with RSI (oversold/overbought)

Exit signals:
1. Profit target (0.15-0.30%)
2. Stop loss (0.10-0.20%)
3. Range break (price exceeds bands)
4. Timeout (5-15 minutes)
...
def range_scalping(df, bb_period=20, bb_std=2.0, rsi_period=14, 
    ...
    for i in range(bb_period + 5, len(df)):
        ...
        if not position:
            ...
            # Long signal: Price at lower BB + RSI oversold
            ...
            # Short signal: Price at upper BB + RSI overbought
...
print("="*80)
print("RANGE SCALPING - MULTI-ASSET CLASS TESTING")
print("="*80)
...
if all_results:
    ...
    if best_result['sharpe'] > 0:
        print(f"\n🎯 **PROFITABLE on {best_symbol}!** Continue with full year validation")
    else:
        print(f"\n❌ No profitable assets - Range Scalping doesn't work")

# Save
with open('range_scalping_multi_asset_results.json', 'w') as f:
    json.dump(all_results, f, indent=2)

print(f"\n✅ Saved to range_scalping_multi_asset_results.json")
