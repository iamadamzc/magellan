"""
ORB V15 "LET IT RUN" - Single Target, No Early Scaling
-------------------------------------------------------
OPPOSITE of V14: Stop chopping up winners.

RADICAL CHANGES:
1. NO scaling until 1.5R
2. Take 50% at 1.5R only
3. Trail remaining 50% with WIDE stop (1.0 ATR)
4. Delay BE to 0.8R (compromise)
5. NO VWAP exit

HYPOTHESIS: The paradox exists because we keep taking tiny profits.
Let's see if ONE good winner at 1.5R+ beats many 0.7R winners.
"""
...
def run_orb_v15_let_it_run(symbol, start, end):
    ...
    params = {
        ...
        # RADICAL: Single target, let it run
        'BREAKEVEN_TRIGGER_R': 0.8,  # Delayed BE
        'PROFIT_TARGET': 1.5,        # ONE target only
        'SCALE_SIZE': 0.50,          # Take 50% at 1.5R
        'TRAIL_ATR': 1.0,            # WIDE trail for runners
    }
    
    print(f"\nTesting {symbol} - V15 LET IT RUN ({start} to {end})")
    ...
    for i in range(len(df)):
        ...
        if position is None:
            ...
            # BREAKEVEN @ 0.8R (delayed)
            if current_r >= params['BREAKEVEN_TRIGGER_R'] and not moved_to_be:
                stop_loss = entry_price
                moved_to_be = True
            
            # SINGLE SCALE @ 1.5R
            if current_r >= params['PROFIT_TARGET'] and not scaled:
                pnl_pct = (risk * params['PROFIT_TARGET']) / entry_price * 100 * params['SCALE_SIZE']
                trades.append({'pnl_pct': pnl_pct, 'r': params['PROFIT_TARGET'], 'type': 'scale_15r'})
                position -= params['SCALE_SIZE']
                scaled = True
            
            # WIDE TRAILING STOP (only after BE)
            if moved_to_be:
