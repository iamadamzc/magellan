"""
ORB V19 "FIRST HOUR ONLY" - The Fix We Should Have Done First
--------------------------------------------------------------
CRITICAL FIX: Only trade during first hour when ORB actually works!

CHANGE FROM V17:
- Entry window: 9:40 AM - 10:30 AM ONLY (50 minutes)
- After 10:30 AM: NO NEW ENTRIES (hold existing positions)

HYPOTHESIS: ORB breakouts at 2 PM are noise. Real ORB edge is in first hour.
This should MASSIVELY improve results.
"""
...
def run_orb_v19_first_hour(symbol, start, end):
    """V19: V7 logic + V17 regime filter + FIRST HOUR ONLY entry window"""
    ...
    params = {
        ...
        # NEW: Entry window restriction
        'ENTRY_WINDOW_START': 10,  # Minutes since open (9:40 AM)
        'ENTRY_WINDOW_END': 60,    # Minutes since open (10:30 AM)
    }
    
    print(f"\nTesting {symbol} - V19 FIRST HOUR ONLY ({start} to {end})")
    ...
    for i in range(len(df)):
        ...
        # Entry logic (WITH TIME WINDOW RESTRICTION)
        if position is None:
            # CRITICAL: Only enter during entry window
