"""
CLEAN-ROOM BACKTEST: STRATEGY A - MSI HOURLY SWING
...
print("=" * 80)
print("CLEAN-ROOM BACKTEST: STRATEGY A - MSI HOURLY SWING")
print("=" * 80)
print("Instrument: MSI (Micro Silver Futures)")
...
def fetch_futures_data(symbol, start_date, end_date, timeframe='1hour'):
    """Fetch futures data from FMP API"""
    import os
    import requests
    ...
    # FMP uses /stable endpoint for futures
    # Format: /historical-chart/{timeframe}/{symbol}
    base_url = "https://financialmodelingprep.com/api/v3"
    endpoint = f"/historical-chart/{timeframe}/{symbol}"
    ...
    print(f"Fetching data from FMP: {url}")
...
for period in TEST_PERIODS:
    ...
    try:
        ...
        df = fetch_futures_data(SYMBOL, period['start'], period['end'], timeframe='1hour')
...
print(f"\n\n{'#' * 80}")
print("STRATEGY A - SUMMARY OF ALL TESTS")
print(f"{'#' * 80}")
...
# Save summary
output_dir = Path(__file__).parent
summary_df.to_csv(output_dir / "summary_strategy_a.csv", index=False)
