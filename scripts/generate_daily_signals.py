...
def analyze_asset(symbol, spy_regime):
    ...
    if bull_entry or strong_entry:
        signal = "BUY/LONG"
        if bull_entry:
            details.append("Bull Regime Entry (RSI > 55, SPY > 200MA)")
        if strong_entry:
            details.append("Strong Breakout (RSI > 65)")
    elif exit_signal:
        signal = "SELL/EXIT"
        if rsi < 45:
            details.append("Momentum Loss (RSI < 45)")
        if sentiment < -0.3:
            details.append("Negative Sentiment")
