...
def run_rolling_backtest(
    ...
    if start_date is not None and end_date is not None:
        ...
        if not quiet:
            print(
                f"[TEMPORAL] Syncing Clock to Epoch: {start_date.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}"
            )
    ...
    if not quiet:
        print(
            f"[STRESS TEST] Date range: {trading_days[0].strftime('%Y-%m-%d')} to {trading_days[-1].strftime('%Y-%m-%d')}"
        )
    ...
    try:
        all_bars = alpaca_client.fetch_historical_bars(
            symbol=symbol, timeframe=target_tf, start=start_str, end=end_str, feed="sip"
        )
        if not quiet:
    ...
    for window_idx in range(num_oos_windows):
        ...
        if len(is_features) < 50 or len(oos_features) < 30:
            log_msg(
                f"  [SKIP] Insufficient features after warmup: IS={len(is_features)}, OOS={len(oos_features)}",
                verbose=True,
            )
            continue
        ...
        is_pnl_metrics = simulate_portfolio(
            is_sim, initial_capital=cumulative_equity, friction_bps=1.5, max_position_dollars=LIQUIDITY_CAP_USD
        )
        ...
        pnl_metrics = simulate_portfolio(
            oos_sim, initial_capital=cumulative_equity, friction_bps=1.5, max_position_dollars=effective_oos_cap
        )
...
def print_stress_test_summary(result: Dict) -> None:
    ...
    log_sum(
        f"  Cumulative P&L:      ${result['cumulative_pnl_dollars']:+,.2f} ({pnl_sign}{result['cumulative_pnl_pct']:.2f}%)"
    )
    ...
    if result["daily_results"]:
        ...
        for day in result["daily_results"]:
            log_sum(
                f"{day['date']:<12} {day['in_sample_hit_rate']*100:>5.1f}%  {day['out_sample_hit_rate']*100:>5.1f}%  {day['wfe']:>5.2f}  ${day['daily_pnl_dollars']:>+10,.2f}"
            )
...
def export_stress_test_results(result: Dict, filepath: str = "stress_test_equity.csv") -> None:
    ...
    equity_df = pd.DataFrame({"equity": result["master_equity_curve"]})
