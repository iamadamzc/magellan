...
class AlpacaTradingClient:
    ...
    def liquidate_all_positions(self) -> dict:
        ...
        for pos in positions:
            ...
            try:
                order = self.api.submit_order(symbol=symbol, qty=abs_qty, side=side, type="market", time_in_force="day")
                ...
                closed_positions.append({"symbol": symbol, "qty": qty, "order_id": order.id, "status": order.status})
...
def get_asymmetric_allocation(iwm_returns: list = None, vss_returns: list = None, base_weight: float = 1.0) -> dict:
    """
...
def execute_trade(
    ...
    if signal == -1:
        ...
        print(
            f"[EXECUTION] {symbol} | Base: ${base_capital:,.2f} | Metabolism: {metabolism_pct}% | Final: ${final_size_usd:,.2f}"
        )
    ...
    try:
        order = client.api.submit_order(
            symbol=symbol, qty=qty, side=side, type="limit", time_in_force="day", limit_price=limit_price
        )
        ...
        while (time.time() - start_time) < 10:
            ...
            try:
                ...
                if current_status == "filled":
                    is_filled = True
                    filled_avg_price = (
                        float(updated_order.filled_avg_price) if updated_order.filled_avg_price else limit_price
                    )
