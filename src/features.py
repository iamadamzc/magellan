...
class FeatureEngineer:
    ...
    @staticmethod
    def merge_all(price_df: pd.DataFrame, fmp_metrics: Dict[str, any], fmp_sentiment: Dict[str, any]) -> pd.DataFrame:
        """
...
def get_damping_factor(df: pd.DataFrame, ticker: str = None, node_config: dict = None) -> dict:
    """
...
def add_wavelet_signals(df: pd.DataFrame, ticker: str = None, node_config: dict = None) -> pd.DataFrame:
    """
...
def carrier_wave_confluence(df: pd.DataFrame, ticker: str = None, node_config: dict = None) -> pd.DataFrame:
    """
...
def merge_news_pit(
    price_df: pd.DataFrame, news_list: list, lookback_hours: int = 4, ticker: str = None
) -> pd.DataFrame:
    """
    ...
    if nlp_engaged:
        LOG.warning(
            f"[PIT] API sentiment missing. Local NLP engaged for {len(news_df)} articles. Latency: {calc_time:.4f}s"
        )
...
def generate_master_signal(df: pd.DataFrame, node_config: dict = None, ticker: str = None) -> pd.DataFrame:
    """
    ...
    for idx in df.index:
        ...
        if ticker == "VSS" and is_cryo_hot:
            ...
            temp_baseline = (
                df["vol_temp"].rolling(window=200).median().loc[idx] if "vol_temp" in df.columns else temp_val
            )
    ...
    # Summary Telemetry
    LOG.stats(
        f"[SIGNAL] Fermi Gate: {fermi_gate:.4f} (Mean={alpha_mean:.4f} + {sigma_multiplier}*StdDev={alpha_std:.4f})"
    )
    ...
    LOG.stats(
        f"[ALPHA] Master signal generated: mean={df['alpha_score'].mean():.4f}, std={df['alpha_score'].std():.4f}"
    )
    # -------------------------------------------------------------------------
