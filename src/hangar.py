...
def calculate_kinetic_potential(current_price: float, friday_close: float, volume_zscore: float) -> float:
    """
...
def analyze_orh_bars(orh_bars: pd.DataFrame, friday_close: float, ticker: str) -> Dict:
    """
    ...
    if len(orh_bars) == 0:
        return {"ticker": ticker, "error": "No ORH bars available", "ep": 0.0, "status": "NO_DATA"}
...
def run_hangar_observation(tickers: List[str], alpaca_client, lookback_days: int = 5) -> Dict[str, Dict]:
    """
    ...
    for ticker in tickers:
        ...
        try:
            ...
            friday_bars = alpaca_client.fetch_historical_bars(
                symbol=ticker, timeframe=TimeFrame.Day, start=friday_str, end=friday_end_str, feed="sip"
            )
            ...
            results[ticker] = {"ticker": ticker, "error": str(e), "ep": 0.0, "status": "ERROR"}
    ...
    for ticker, result in results.items():
        if "error" not in result:
            print(
                f"{ticker:6} | Ep: {result['ep']:+8.4f} | Status: {result['status']:10} | "
