...
class OptionsFeatureEngineer:
    ...
    def calculate_black_scholes_greeks(
        ...
        # Theta (daily decay)
        if option_type.lower() in ["call", "c"]:
            theta = (-(S * n_d1 * sigma) / (2 * np.sqrt(T)) - r * K * np.exp(-r * T) * N_d2) / 365  # Convert to daily
        else:  # put
    ...
    @staticmethod
    def calculate_iv_rank(current_iv: float, iv_history: pd.Series, lookback_days: int = 252) -> float:
        """
    ...
    @staticmethod
    def calculate_iv_percentile(current_iv: float, iv_history: pd.Series, lookback_days: int = 252) -> float:
        """
    ...
    def estimate_iv_from_price(
        ...
        for i in range(max_iterations):
            ...
            greeks = OptionsFeatureEngineer.calculate_black_scholes_greeks(S, K, T, r, sigma, option_type)
    ...
    def should_roll_position(
        dte: int, roll_threshold_dte: int = 7, current_pnl_pct: Optional[float] = None, min_profit_to_roll: float = 0.0
    ) -> bool:
        """
    ...
    def calculate_position_size_contracts(
        target_notional: float, current_price: float, delta: float, min_contracts: int = 1, max_contracts: int = 100
    ) -> int:
        """
        ...
        LOG.flow(f"Position size: {contracts} contracts " f"(Target: ${target_notional:,.0f}, Delta: {delta:.2f})")

        return contracts

    @staticmethod
    def calculate_breakeven_move_pct(premium: float, strike: float, option_type: str) -> float:
        """
    ...
    @staticmethod
    def estimate_win_probability(S: float, K: float, T: float, sigma: float, option_type: str = "call") -> float:
        """
