...
def simulate_portfolio(
    df: pd.DataFrame, initial_capital: float = 100000.0, friction_bps: float = 0.0, max_position_dollars: float = None
) -> Dict:
    """
...
def print_virtual_trading_statement(metrics: Dict) -> None:
    ...
    print(
        f"Total Return:           ${metrics['total_return_dollars']:,.2f} ({ret_sign}{metrics['total_return_pct']:.2f}%)"
    )
