...
class VolatilityTargeter:
    ...
    def calculate_scaling_factor(self, recent_returns: Optional[pd.Series] = None) -> dict:
        """
        ...
        if recent_returns is not None:
            ...
            if len(self.portfolio_returns) < self.lookback_days:
                ...
                return {"scaling_factor": 1.0, "realized_vol": self.target_vol, "target_vol": self.target_vol}
        ...
        if realized_vol == 0 or np.isnan(realized_vol):
            ...
            return {"scaling_factor": 1.0, "realized_vol": 0.0, "target_vol": self.target_vol}
        ...
        return {"scaling_factor": scaling, "realized_vol": realized_vol, "target_vol": self.target_vol}
...
def get_volatility_scaling(portfolio_returns: pd.Series, target_vol: float = 0.15, lookback_days: int = 20) -> dict:
    """
    ...
    if len(portfolio_returns) < lookback_days:
        ...
        return {"scaling_factor": 1.0, "realized_vol": target_vol, "target_vol": target_vol}
    ...
    if realized_vol == 0 or np.isnan(realized_vol):
        LOG.warning("[VOL TARGET] Zero or NaN realized volatility, using full size")
        return {"scaling_factor": 1.0, "realized_vol": 0.0, "target_vol": target_vol}
    ...
    LOG.info(
        f"[VOL TARGET] Realized: {realized_vol*100:.1f}% | Target: {target_vol*100:.1f}% | Scaling: {scaling:.2f}x"
    )

    return {"scaling_factor": scaling, "realized_vol": realized_vol, "target_vol": target_vol}
