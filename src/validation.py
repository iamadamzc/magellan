...
def run_walk_forward_check(
    df: pd.DataFrame, alpha_col: str, target_col: str = "log_return", in_sample_pct: float = 0.70, horizon: int = 15
) -> Dict:
    """
...
def run_optimized_walk_forward_check(
    ...
    opt_result = optimize_alpha_weights(
        in_sample, feature_cols=feature_cols, target_col=target_col, horizon=0, metric="hit_rate"  # Already shifted
    )
