...
Edit: `deployed/daily_trend/config.json`
...
Same process for `deployed/hourly_swing/config.json` and `magellan-hourly-swing.service`
...
# Verify files exist
ls -la deployed/bear_trap/
ls -la deployed/daily_trend/
ls -la deployed/hourly_swing/
```
...
sudo cp deployed/bear_trap/magellan-bear-trap.service /etc/systemd/system/
sudo cp deployed/daily_trend/magellan-daily-trend.service /etc/systemd/system/
sudo cp deployed/hourly_swing/magellan-hourly-swing.service /etc/systemd/system/
...
ls -la /home/ec2-user/magellan/deployable_strategies/bear_trap/runner.py
