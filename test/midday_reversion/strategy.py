...
class MiddayReversionStrategy:
    """
    Midday Reversion Strategy (Research-Based)

    Entry Criteria:
    - Stock down ≥10% from session open (lowered from -15%)
    - Time: 11:30 AM - 2:00 PM ET (midday window = 60% win rate)
    - Optional filters:
      * SPY > 0% (market tailwind)
      * Above 200 SMA (trend filter)
      * Not near 52-week low (avoid falling knives)
    - Reclaims above session low with quality candle
    ...
    - Expected recovery: ~6%
    - Sample: 3,514 events over 5 years
    """
    ...
    def __init__(
        ...
        self.logger = logging.getLogger("magellan.midday_reversion")
        self.trade_logger = TradeLogger(strategy_name="midday_reversion")
    ...
    def _evaluate_entry(self, symbol: str, current, df):
        ...
        if not is_down_enough:
            self.trade_logger.log_decision(
                ...
                details=f"Required: -10%, Current: {day_change:.1f}%",
    ...
    def _manage_position(self, symbol: str, current, df):
        ...
        # Check time stop (60 minutes - extended for midday strategy)
