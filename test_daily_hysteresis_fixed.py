...
print(f"[OK] Fetched {len(bars)} daily bars")
...
print(f"[OK] RSI calculated ({len(bars)} bars after warmup)")
...
print(f"[OK] Signals generated: {num_trades} trades")
...
print(f"\n[SAVED] Equity curve saved to: variant_f_equity_curve.csv")

# Verdict
print("\n" + "=" * 70)
if ENABLE_HYSTERESIS:
    if strategy_return > 0 and num_trades < 100 and max_drawdown > -20:
        print("[SUCCESS] VARIANT F SUCCESS: Hysteresis reduced whipsaw and captured trend")
    elif strategy_return > 0:
        print("[WARNING]  PARTIAL SUCCESS: Profitable but check trade count or drawdown")
    else:
        print("[FAIL] VARIANT F FAILED: Hysteresis did not solve whipsaw problem")
else:
    print("[STATS] BASELINE: Reference performance for comparison")
print("=" * 70)
